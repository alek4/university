...
/*
1. no, "uso una struttura ad albero" non descrive che tipo di albero utilizzerà e il significato dei nodi e delle loro relazioni. Inoltre, la modellazione del problema è sbagliata, per poter risolvere il problema è necessario utilizzare una struttura dati che permetta di implementare una foresta.
2. no, nel commento vengono date per scontate molte cose, non viene descritto a fondo il perchè vengono utilizzate diverse implementazioni.
	correzione: Non sapendo quanti subordinati ha un dipendente, ho usato una mappa per i subordinati di una struttura dipendente (struttura con nome, e puntatore al superiore)
e una lista contenente tutti i dipendenti in elenco.
*/
